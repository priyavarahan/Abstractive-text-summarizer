...
Original file is located at
    https://colab.research.google.com/drive/1_tCycVzMe2UBjOqnGkPbk6av_ebFhkjQ
"""
...
# Define paths
dataset = pd.read_csv('/content/drive/MyDrive/CNN.csv')
...
def cleaner_txt(txt, num):
    string = txt.lower()  # Convert to lowercase
    ...
    string = re.sub(r"[^a-zA-Z]", " ", string)  # Remove non-alphabetic characters
    string = re.sub(r'[m]{2,}', 'mm', string)  # Replace consecutive 'm' characters
    if num == 0:
        tokens = [w for w in string.split() if not w in stop_words]  # Remove stopwords if num is 0
    else:
        tokens = string.split()
    long_words = [i for i in tokens if len(i) > 1]  # Filter out short words
...
# fix the maximum length of review to
max_len_txt=512
max_len_summary=150
...
# Print ROUGE scores for validation set
print("Validation ROUGE-1 Score:", val_scores["rouge-1"])
print("Validation ROUGE-2 Score:", val_scores["rouge-2"])
print("Validation ROUGE-L Score:", val_scores["rouge-l"])

# Print ROUGE scores for training set
print("Training ROUGE-1 Score:", train_scores["rouge-1"])
print("Training ROUGE-2 Score:", train_scores["rouge-2"])
print("Training ROUGE-L Score:", train_scores["rouge-l"])
